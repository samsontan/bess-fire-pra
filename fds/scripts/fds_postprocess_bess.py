...
# PATHS
# =============================================================================
FDS_DIR = r"C:\FDS_runs\eqix_preview"
CHID    = "eqix_preview"
OUT_DIR = os.path.join(FDS_DIR, "figures")
...
# =============================================================================
# MESH GEOMETRY (hardcoded for eqix_preview two-mesh setup)
...
def add_cabinet_patches_plan(ax):
    ...
    ax.text(3.75, 4.87, 'Supply (OPEN)', ha='center', va='center',
            fontsize=6, color='#006600', zorder=6)
    # South aisle label
    ax.text(3.75, 1.00, 'South aisle', ha='center', va='center',
            fontsize=6, color='#555555', style='italic', zorder=6)
...
def make_fig14(smv_mapping, fds_dir, chid):
    ...
    for ax, t_target in zip(axes, snaps):
        ...
        for lvl, col, lbl in [(TLV_C, 'dodgerblue', f'{TLV_C} ppm'),
            ...
            if np.nanmax(hf_ppm) > lvl:
                ...
                try:
                    ax.clabel(cs, fmt={lvl: lbl}, fontsize=6.5, inline=True)
                except Exception:
                    pass
        ...
        if t_act < t_target - 5:
            t_str += ' (latest)'
        ax.set_title(t_str, fontsize=9, fontweight='bold')
    ...
    cb.set_label('HF concentration (ppm)', fontsize=8)
...
def make_fig12(smv_mapping, fds_dir, chid):
    ...
    for ax, t_target in zip(axes, snaps):
        ...
        if all_temp is not None:
            ...
            temp_levels = [50, 100, 200, 400, 800]
            try:
                ct = ax.contour(XT, ZT, T_plot, levels=temp_levels,
                                colors=['#FF8800', '#FF4400', '#CC0000', '#880000', '#440000'],
                                linewidths=1.0, zorder=6)
                ax.clabel(ct, fmt='%d C', fontsize=6.5, inline=True)
            except Exception:
                pass
        ...
        if t_act < t_target - 5:
            t_str += ' (latest)'
        ax.set_title(t_str, fontsize=9, fontweight='bold', pad=3)
        ...
        cb.set_label('HF (ppm)', fontsize=7)
        for lvl, col in [(TLV_C, 'dodgerblue'), (IDLH, 'darkred')]:
    ...
    for label, key, threshold, thr_label, cmap_name in [
        ...
        for ax, t_target in zip(axes, snaps):
            ...
            if np.nanmax(data) > threshold:
                ...
                try:
                    ax.clabel(cs, fmt={threshold: thr_label}, fontsize=6.5, inline=True)
                except Exception:
                    pass
            add_cabinet_patches_plan(ax)
...
def make_fig_h2_bz(smv_mapping, fds_dir, chid):
    ...
    sm.set_array([])
    cb = fig.colorbar(sm, ax=axes, fraction=0.025, pad=0.02, aspect=25)
    cb.set_label('H2 mass fraction (kg/kg)', fontsize=8)
    fig.suptitle('H2 Concentration at Breathing Zone (Z = 1.50 m)\n'
                 'EQIX SG4-4A BESS Room 1 | 9 ACH Stage 0 Purge',
                 fontsize=10, fontweight='bold')
...
def make_fig_fire_section(smv_mapping, fds_dir, chid):
    ...
    for r_idx, (qty_label, (times_q, all_data, h_vec, v_vec)) in enumerate(datasets.items()):
        ...
        for c_idx, t_target in enumerate(snaps):
            ...
            if qty_label == 'TEMP':
                for lvl, col in [(100, '#FF8800'), (300, '#FF2200'), (600, '#880000')]:
                    if frame.max() > lvl:
                        ...
                        try:
                            ax.clabel(cs, fmt=f'{lvl}C', fontsize=6, inline=True)
                        except Exception:
                            pass
            elif qty_label == 'HF':
                for lvl, col, lbl in [(TLV_C, 'dodgerblue', f'{TLV_C} ppm'),
                    ...
                    if frame.max() > lvl:
                        ...
                        try:
                            ax.clabel(cs, fmt={lvl: lbl}, fontsize=6, inline=True)
                        except Exception:
                            pass
            ...
            if c_idx == 0:
                ax.set_ylabel(f'Z (m)\n{row_titles[qty_label]}', fontsize=8)
    ...
    fig.suptitle('Vertical Section X = 3.24 m (through CAB-4 fire) | EQIX SG4-4A BESS\n'
...
def make_device_plot(sim):
    ...
    for (dev_id, dev), col in zip(hf_devs.items(), line_colors):
        try:
            ...
            ax.plot(time_arr, ppm_arr, color=col, linewidth=1.5, label=dev_id)
        except Exception as e:
            print(f"  WARNING: {dev_id}: {e}")

    ax.axhline(IDLH,  color='red',    linewidth=1.2, linestyle='-',  alpha=0.7,
               label=f'IDLH = {IDLH} ppm')
    ax.axhline(TLV_C, color='orange', linewidth=1.0, linestyle='--', alpha=0.7,
               label=f'STEL = {TLV_C} ppm')
    ax.axhline(0.5,   color='#6699FF', linewidth=0.8, linestyle=':',  alpha=0.7,
               label='Ceiling = 0.5 ppm')

    ax.set_xlabel('Time (s)', fontsize=10)
    ax.set_ylabel('HF concentration (ppm)', fontsize=10)
    ax.set_title('HF Sensor Time History -- EQIX SG4-4A BESS Room 1\n'
                 '9 ACH Stage 0 Purge | NMC Galaxy LBF 242.76 kWh',
                 fontsize=10, fontweight='bold')
    ...
    for ax, t_target in zip(axes, snaps):
        ...
        for lvl, col in [(50, '#FFEE88'), (100, '#FFAA00'),
            ...
            if np.nanmax(data) > lvl:
                ...
                try:
                    ax.clabel(cs, fmt=f'{lvl}C', fontsize=6.5, inline=True)
                except Exception:
                    pass
